...
TMCRA TokenGraph-LLM Stage C 不计算完整序列内所有 token pair 的 attention。它使用候选 token graph 和学习式 edge gate。设：
...
| TMCRA TokenGraph-LLM graph encoder | 沿候选边 message passing | 每层 `O((N + E) * d)` |
| TMCRA TokenGraph-LLM dynamic prefix decoder | 有限窗口 generated-token graph | 每层 `O(T * W * d)` |
| TMCRA TokenGraph-LLM context tunnel decoder | generated token 到 encoded context graph | 每层 `O(T * N * d)` |

所以当前优势不是“生成恒定成本”。准确说法是：TMCRA TokenGraph-LLM 用 typed graph candidate edges、learned edge activation、bounded prefix graph messages 和 explicit context tunneling，替代 Transformer 的 dense sequence-wide self-attention。
